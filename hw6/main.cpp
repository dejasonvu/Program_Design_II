...
int main(int argc, char** argv){
    ...
    //calculating idf and preparing the data for output
    int k = stoi(argv[3]);
    for(auto &i : que){
        unordered_map<long long, double> idf_sum;
        for(auto &j : i.words){
            if(directionary[j].size() == 0){
                continue;
            }
            double idf = log10((double)num_strings / directionary[j].size());
            if(idf == 0){
                continue;
            }
        ...
        for(int t=0; t<k; t++){
            ...
            long long rec = idf_sum.begin()->first;
            for(auto &it : idf_sum){
                if(it.second > idf_sum[rec]){
                    rec = it.first;
                }else if(it.second == idf_sum[rec]){
                    if(it.first < rec){
                        rec = it.first;
                    }
                }
